...
const PSU_WATT_OPTIONS: ProductFilterOption[] = [
  { label: 'Todos', value: '' },
  ...['450', '500', '550', '600', '650', '700', '750', '800', '850', '1000', '1200', '1500'].map((value) => ({
    label: `${value} W`,
    value,
  })),
];
...
export const PRODUCT_FILTERS_BY_CATEGORY: Record<string, ProductFilterConfig[]> = {
  ...
  MOTHERBOARD: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['ASUS', 'MSI', 'Gigabyte', 'ASRock', 'Otros']) },
  ...
  RAM: [
    ...
    { key: 'capacity', label: 'Capacidad', type: 'select', options: withAll(['8', '16', '24', '32']) },
  ...
  GPU: [
    { key: 'brand', label: 'Marca ensambladora', type: 'select', options: withAll(['Gigabyte', 'ASUS', 'MSI', 'PNY', 'Otros']) },
    { key: 'gpuChipset', label: 'Chipset', type: 'select', optionSource: 'gpuChipsets', options: withAll(['NVIDIA', 'AMD', 'Intel']) },
    { key: 'vram', label: 'VRAM', type: 'select', optionSource: 'vram', options: withAll(['4', '6', '8', '12', '16', '24', '32']) },
  ],
  PSU: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['MSI', 'ASUS', 'Gigabyte', 'Corsair', 'DeepCool', 'Antryx', 'Cooler Master', 'Seasonic', 'Thermaltake', 'Otros']) },
    { key: 'psuWatts', label: 'Potencia', type: 'select', options: PSU_WATT_OPTIONS },
  ],
  COOLER: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['MSI', 'DeepCool', 'Corsair', 'Gigabyte', 'ASUS', 'Otros']) },
  ...
  STORAGE: [
    { key: 'brand', label: 'Marca', type: 'select', optionSource: 'brands', options: withAll(['Kingston', 'Samsung', 'Western Digital', 'Crucial', 'Seagate', 'Otra']) },
  ...
  CASE: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Halion', 'Micronics', 'ASUS', 'Gigabyte', 'DeepCool', 'Antryx', 'MSI', 'Lian Li', 'Otros']) },
  ...
  LAPTOP: [
    ...
    { key: 'screenSize', label: 'Tamano de pantalla', type: 'select', options: LAPTOP_SCREEN_OPTIONS },
  ...
  MONITOR: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['MSI', 'Gigabyte', 'Teros', 'LG', 'Samsung', 'Otros']) },
  ...
  KEYBOARD: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Redragon', 'MSI', 'Logitech', 'Razer', 'Aula', 'Royal Kludge', 'Otros']) },
    ...
    { key: 'connections', label: 'Conectividad', type: 'select', optionSource: 'connections', options: withAll(['Cableado', 'Bluetooth', 'Dongle USB']) },
    { key: 'layoutLanguage', label: 'Idioma / Layout', type: 'select', options: withAll(['Espanol', 'Ingles']) },
    { key: 'hasLighting', label: 'RGB', type: 'select', options: yesNoOptions() },
  ],
  MOUSE: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Redragon', 'Logitech', 'Razer', 'MSI', 'Teros', 'Otros']) },
    { key: 'connections', label: 'Conexion', type: 'select', optionSource: 'connections', options: withAll(['Cableado', 'Bluetooth', 'Dongle USB']) },
  ],
  MOUSEPAD: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['HyperX', 'Logitech', 'Redragon', 'Otros']) },
  ],
  CHAIR: [
  ...
  WEBCAM: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Logitech', 'Redragon', 'Otros']) },
    { key: 'resolution', label: 'Resolucion', type: 'select', options: withAll(['HD', 'FHD', '4K']) },
    { key: 'fps', label: 'FPS', type: 'select', options: withAll(['30', '60']) },
  ],
  CAPTURE_CARD: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Corsair', 'Streamplify', 'Otros']) },
    { key: 'resolution', label: 'Resolucion', type: 'select', options: withAll(['HD', 'FHD', '4K']) },
    { key: 'fps', label: 'FPS', type: 'select', options: withAll(['30', '60', '120']) },
  ],
  CABLE_HUB: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Cabletime', 'Ugreen', 'Otros']) },
    { key: 'cableHubType', label: 'Tipo', type: 'select', options: withAll(['Cable', 'Hub']) },
  ],
  LAPTOP_COOLING_BASE: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Cooler Master', 'Antryx', 'Teros', 'Otros']) },
  ],
  BACKPACK: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Redragon', 'ASUS', 'Teros', 'Gigabyte', 'Otros']) },
  ],
  HEADSET: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Logitech', 'Redragon', 'HyperX', 'Razer', 'Teros', 'Otros']) },
    { key: 'connection', label: 'Conexion', type: 'select', options: withAll(['Cableado', 'Inalambrico']) },
  ],
  MICROPHONE: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Fifine', 'Streamplify', 'Redragon', 'Razer', 'Logitech', 'Corsair', 'Otros']) },
  ],
  SPEAKER: [
    { key: 'brand', label: 'Marca', type: 'select', options: withAll(['Logitech', 'Redragon', 'Creative', 'Genius', 'Otros']) },
  ],
};

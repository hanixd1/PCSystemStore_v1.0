...
describeWithTestDb('Checkout y stock HTTP E2E real', () => {
  ...
  it('STOCK-E2E-01 cliente no puede comprar producto con stock 0', async () => {
    const product = await ctx.prisma.product.findUniqueOrThrow({ where: { sku: qaSkus.stockZero } });

    await request(ctx.app.getHttpServer())
      .post('/orders')
      .set('Authorization', `Bearer ${ctx.tokens.customer}`)
      .send({ method: 'CARD_CREDIT', frontendPrice: 1, items: [{ productId: product.id, quantity: 1 }] })
      .expect(400);
  });
  ...
  it('STOCK-E2E-02 cliente no puede comprar cantidad mayor al stock', async () => {
    ...
    await request(ctx.app.getHttpServer())
      .post('/orders')
      .set('Authorization', `Bearer ${ctx.tokens.customer}`)
      .send({ method: 'CARD_CREDIT', frontendPrice: 1, items: [{ productId: product.id, quantity: 2 }] })
      .expect(400);
  });
  ...
  it('STOCK-E2E-06 pago simulado aprobado descuenta stock', async () => {
    ...
    await request(ctx.app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${ctx.tokens.customer}`)
      .send({ orderId: order.body.id, method: 'CARD_CREDIT', simulateResult: 'APPROVED' })
      .expect(201);
